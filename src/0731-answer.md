...
每执行一轮，`x` 都多乘一个 2。执行 `t` 轮以后，`x` 大约是 `2^(t+1)`；循环在它达到 `n/2` 量级时停止。因此问题变成：2 连乘多少次能够到达 `n`？答案是约 `log₂n` 次，所以选 **A，O(log n)**。
...
`O(log n)` 要求参数类似 `n/2`，这样才能不断减半；`O(n log n)` 通常需要对数层中每层再完成线性工作；`O(n²)` 则需要每层还有一个随 `n` 增长的循环。本题都没有。
...
它的主要增长部分是 `t²`。循环在 `sum` 达到 `n` 时停止，因此反过来解：

```text
t² ≈ n  →  t ≈ √n
```

程序运行的是 `t` 轮，所以选 **B，O(√n)**。这里不能看到 `i++` 就选 `O(n)`，因为 `i` 并不需要走到 `n`；不断累加的 `sum` 会先达到 `n`。
...
因此 `x` 从 0 逐一走到约 `√n`，一共执行 `O(√n)` 轮，所以选 **B**。

这道题有两个方向相反的坑。一个是看到 `x++` 就选 `O(n)`，忘了先求它实际走到哪里；另一个是看到平方就选 `O(n²)`，把停止条件中的平方误当成工作量的平方。平方在这里让循环更早停止。
...
如果真的是一共 `log n` 轮，并且每轮固定做 `log n` 次，结果才是 `O(log² n)`。本题不是这种结构。
...
从代码设计角度看，它也能反过来理解：如果第 `i` 轮做 `i` 次，外层走到 `t` 时总量约为 `t²`；若希望总量约为 `n`，便令 `t ≈ √n`，代码中可以用 `i*i<=n` 表达这个边界。这不是说真实算法通常为了凑复杂度而写，而是帮助建立“边界、更新方式、每轮工作量”如何共同决定总工作量的代码感觉。
...
`Θ(log² n)` 的增长确实严格慢于 `n`，所以数学上有 `log² n = o(n)`，也有 `log² n = O(n)`。但复杂度选择题默认寻找最紧的量级；若程序实际是 `Θ(log² n)`，就不能用宽泛的 `O(n)` 代替准确答案。
