...
源码必须允许第一次接触该实现的人直接进入，不能把“先学会”当作读懂注释的前提。每份源码固定说明四层信息：

1. 文件开头说明本例解决什么问题、采用哪种下标或指针约定；
2. 数据定义说明每个状态字段保存什么，空、满或终止状态怎样表示；
3. 函数前说明输入、结果以及函数结束后应成立的不变量；
4. 核心循环内部说明一轮开始时已经完成什么、本轮改变什么、为什么不会丢失后续信息。

注释解释的是代码背后的状态变化，不重复“给变量加一”之类从语句表面即可看出的内容。题目常换函数名和写法，因此源码优先使用“前驱、未处理区、已确定集合、候选值”等可以迁移到陌生代码的语言。
